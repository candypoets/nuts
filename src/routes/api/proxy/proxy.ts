...
export async function proxyRequest({ url, request }: RequestEvent): Promise<Response> {
	...
	// Use 127.0.0.1 instead of localhost to avoid IPv6 fallback delays
	...
	try {
		const response = await fetch(targetUrl, {
			...
			// @ts-ignore - keepalive is supported in Node.js 18+ fetch
			keepalive: true // Reuse connections when possible
		});
