...
export function catalogUsesQrFulfillment(event: ParsedEvent) {
	...
	if (type === 'event_access' || type === 'pass' || type === 'membership') return true;
...
export function isCatalogDefinition(event: ParsedEvent) {
	if (event.kind() !== 30009 || !event.pubkey() || !event.id() || !catalogD(event)) return false;
	const type = catalogType(event);
	...
	if (type === 'membership' && (!catalogBilling(event) || rawMaxUses !== undefined)) return false;
	return true;
}
...
export function buildCatalogDefinitionTags(definition: CatalogDefinitionInput): string[][] {
	...
	if (definition.type === 'product') {
		...
		if (!includesValue(MEMBERSHIP_BILLING, billing)) throw new Error('billing is invalid');
