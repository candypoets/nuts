...
// 	// }
// 	// remove the proofs that has been spent from storage
// 	get(db).proofs.bulkDelete(proofsToSend.map((p) => p.secret));
// 	// add them to the spent proofs table
// 	get(db).spentProofs.bulkAdd(proofsToSend.map((p) => p));

// 	get(db).proofs.bulkAdd(returnChange);

// 	//@todo add the tokens that are being sent to pending
...
// 			send: get(isEncrypted) ? [] : send.map((s) => s.secret),
// 			returnChange: get(isEncrypted) ? [] : returnChange.map((r) => r.secret)
// 		}
// 	});
// 	return encodedToken;
// };
...
// 		cashuMint,
// 		keys /*seedPhrase ? seedPhrase : undefined*/
// 	);
// 	let count = undefined;
