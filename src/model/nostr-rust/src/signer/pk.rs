...
impl PrivateKeySigner {
    ...
    pub fn new(private_key_hex: &str) -> Result<Self> {
        ...
        info!("Created new PrivateKeySigner with public key: {}", keys.public_key().to_hex());
    ...
    pub fn from_nsec(nsec: &str) -> Result<Self> {
        ...
        info!("Created new PrivateKeySigner from nsec with public key: {}", keys.public_key().to_hex());
    ...
    pub fn generate() -> Self {
        ...
        info!("Generated new PrivateKeySigner with public key: {}", keys.public_key().to_hex());
...
impl Signer for PrivateKeySigner {
    ...
    fn sign_event(&self, event: &mut Event) -> Result<()> {
        debug!("Signing event of kind {} with public key {}", event.kind, self.keys.public_key());
        ...
        // Update the original event with signed data
        *event = signed_event;
    ...
    fn nip04_encrypt(&self, recipient_pubkey: &str, plaintext: &str) -> Result<String> {
        ...
        debug!("Encrypting message using NIP-04 for recipient: {}", recipient_pubkey);
    ...
    fn nip04_decrypt(&self, sender_pubkey: &str, ciphertext: &str) -> Result<String> {
        ...
        debug!("Decrypting message using NIP-04 from sender: {}", sender_pubkey);
    ...
    fn nip44_encrypt(&self, recipient_pubkey: &str, plaintext: &str) -> Result<String> {
        ...
        debug!("Encrypting message using NIP-44 for recipient: {}", recipient_pubkey);
    ...
    fn nip44_decrypt(&self, sender_pubkey: &str, ciphertext: &str) -> Result<String> {
        ...
        debug!("Decrypting message using NIP-44 from sender: {}", sender_pubkey);
    ...
    fn create_and_sign_event(&self, kind: i32, content: &str, tags: Vec<Vec<String>>) -> Result<Event> {
        debug!("Creating and signing event of kind {} with {} tags", kind, tags.len());
            ...
            .filter_map(|tag| {
                if tag.is_empty() {
                    ...
                    if let Some(first) = tag.get(0) {
                        let values = if tag.len() > 1 { tag[1..].to_vec() } else { vec![] };
...
mod tests {
    ...
    fn test_create_and_sign_event() {
        ...
        let event = signer.create_and_sign_event(1, "Test content", tags).unwrap();
