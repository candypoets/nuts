...
impl RelayConnection {
    ...
    pub async fn stats(&self) -> ConnectionStats {
        ...
        stats.active_subscriptions = self.active_subscriptions.read().await.len();
        stats.active_publishes = self.active_publishes.read().await.len();
        stats
    }

    /// Get the number of active operations (subscriptions + publishes)
    pub async fn operation_count(&self) -> usize {
        let subs = self.active_subscriptions.read().await.len();
        let pubs = self.active_publishes.read().await.len();
        subs + pubs
    }
    ...
    pub async fn add_publish(&self, event_id: String, publish_id: String) {
        let mut pubs = self.active_publishes.write().await;
    ...
    pub async fn active_publishes(&self) -> HashMap<String, String> {
        self.active_publishes.read().await.clone()
    }
    ...
    async fn setup_message_handling(&self) -> Result<(), RelayError> {
        ...
        let outgoing_rx = {
            let mut outgoing_guard = self.outgoing_rx.write().await;
            outgoing_guard.take()
        };
        ...
        {
            let mut incoming_guard = self.incoming_rx.write().await;
        ...
        {
            let mut abort_guard = connection_abort.write().await;
        ...
        let outgoing_task = async move {
            while let Some(message) = outgoing_rx.recv().await {
                ...
                if let Err(e) = ws_sink.send(Message::Text(json)).await {
                    ...
                    let mut status = outgoing_status.write().await;
                    *status = ConnectionStatus::Failed;
                    break;
                }
                ...
                {
                    let mut stats = outgoing_stats.write().await;
                    stats.events_published += 1;
                }
                {
                    let mut activity = outgoing_activity.write().await;
                    *activity = instant::Instant::now();
                }
            }
        };
            ...
            while let Some(message) = ws_stream.next().await {
                match message {
                    Ok(Message::Text(text)) => {
                        ...
                        match RelayMessage::from_json(&text) {
                            Ok(relay_msg) => {
                                ...
                                // Update stats and activity
                                {
                                    let mut stats = incoming_stats.write().await;
                                    stats.events_received += 1;
                                }
                                {
                                    let mut activity = incoming_activity.write().await;
                                    *activity = instant::Instant::now();
                                }

                                if let Err(e) = incoming_tx.send(response) {
                    ...
                    Err(e) => {
                        ...
                        let mut status = incoming_status.write().await;
                        *status = ConnectionStatus::Failed;
                        break;
                    }
                }
    ...
    pub async fn close(&self) -> Result<(), RelayError> {
        ...
        if let Some(abort_handle) = self.connection_abort.write().await.take() {
            abort_handle.abort();
        }
...
impl Drop for RelayConnection {
    fn drop(&mut self) {
        ...
        spawn_local(async move {
            ...
            // Abort connection tasks
            if let Some(abort_handle) = connection_abort.write().await.take() {
                abort_handle.abort();
            }

            // Close WebSocket
            {
                let mut ws_guard = websocket.write().await;
                if let Some(ws) = ws_guard.take() {
                    let _ = ws.close(None, None);
                }
            }
