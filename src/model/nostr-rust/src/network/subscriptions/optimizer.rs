    ...
    fn group_requests_by_relay(&self, requests: Vec<Request>) -> HashMap<String, Vec<Request>> {
        ...
        for request in requests {
            for relay in &request.relays {
                relay_groups.entry(relay.clone()).or_insert_with(Vec::new).push(request.clone());
            }
        }
...
impl SubscriptionOptimizerTrait for SubscriptionOptimizer {
    fn optimize_subscriptions(&self, requests: Vec<Request>) -> Vec<Request> {
        ...
        for (relay, relay_requests) in relay_groups {
            debug!("Optimizing {} requests for relay: {}", relay_requests.len(), relay);
            ...
            let mut relay_optimized: Vec<Request> = merged_requests.into_iter()
                .map(|mut req| {
        ...
        info!("Optimized subscription requests: {} -> {}", 
              request_count, deduplicated.len());
