use crate::db::types::{
    ProcessedNostrEvent, DatabaseConfig, DatabaseError, EventStorage,
    intersect_event_sets, union_event_sets,
};
...
async fn test_add_and_get_event() {
    ...
    let retrieved = db.get_event(&event_id).await;
...
async fn test_query_with_time_range() {
    ...
    db.add_event(create_test_parsed_event(old_event.clone())).await.unwrap();
    db.add_event(create_test_parsed_event(new_event.clone())).await.unwrap();
...
async fn test_query_with_search() {
    ...
    db.add_event(create_test_parsed_event(event1)).await.unwrap();
    db.add_event(create_test_parsed_event(event2)).await.unwrap();
    db.add_event(create_test_parsed_event(event3)).await.unwrap();
...
async fn test_complex_query() {
    ...
    db.add_event(create_test_parsed_event(event.clone())).await.unwrap();
...
async fn test_query_events_for_requests() {
    ...
    let (_, filtered_events) = db
        .query_events_for_requests(requests, true)
        .await
        .unwrap();
