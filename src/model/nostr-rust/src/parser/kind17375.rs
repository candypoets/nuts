...
impl Parser {
    pub fn parse_kind_17375(&self, event: &Event) -> Result<(Kind17375Parsed, Option<Vec<Request>>)> {
        ...
        //                                 if let Ok(pub_key) = nostr::PublicKey::from_secret_key(&nostr::SecretKey::from_hex(&tag[1])) {
        //                                     parsed.p2pk_pub_key = Some(pub_key.to_hex());
        //                                 }
        //                             }
        //                             _ => {}
        //                         }
    ...
    pub fn prepare_kind_17375(&self, event: &mut Event) -> Result<()> {
        ...
        //     Ok(())
        // } else {
        //     Err(anyhow!("no signer available to encrypt wallet data"))
        // }
...
mod tests {
    ...
    fn test_prepare_kind_17375_invalid_content() {
        ...
        assert!(result.unwrap_err().to_string().contains("invalid wallet content"));
    }

    #[test]
    fn test_prepare_kind_17375_missing_mint() {
        let keys = Keys::generate();
        let content = r#"[["privkey", "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"]]"#;
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("wallet must include at least one mint"));
    }
    ...
    fn test_prepare_kind_17375_missing_privkey() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("wallet must include a private key"));
    }
    ...
    fn test_prepare_kind_17375_invalid_privkey() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("private key appears invalid"));
    }
