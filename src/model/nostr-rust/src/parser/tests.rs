...
mod tests {
    ...
    fn test_get_relays_kind_0() {
        ...
        let relays = parser.get_relays(0, "test_pubkey", &false);
    ...
    fn test_get_relays_kind_10002() {
        let indexer_relays = vec![
            "wss://indexer1.com".to_string(),
            "wss://indexer2.com".to_string(),
        ];
        let parser = Parser::new(vec![], indexer_relays);

        let relays = parser.get_relays(10002, "test_pubkey", &false);
    ...
    fn test_get_relays_other_kinds() {
        ...
        let relays = parser.get_relays(1, "test_pubkey", &false);
    ...
    fn test_parse_kind_3() {
        ...
        let tags = vec![Tag::parse(vec![
            "p".to_string(),
            contact_key.public_key().to_string(),
    ...
    fn test_parse_kind_4() {
        ...
        let parsed_value = parsed_event.parsed.unwrap();
        assert_eq!(
            parsed_value["recipient"],
            recipient_keys.public_key().to_hex()
        );
        assert!(parsed_value["chatId"]
            .as_str()
            .unwrap()
    ...
    fn test_parse_kind_10019() {
        ...
        assert_eq!(
            parsed_value["trustedMints"][0]["baseUnits"],
            json!(["sat", "usd"])
        );
        assert_eq!(parsed_value["p2pkPubkey"], pubkey);
        assert_eq!(
            parsed_value["readRelays"],
            json!(["wss://relay.example.com"])
        );
    }
    ...
    fn test_content_parsing_integration() {
        ...
        let has_hashtag = blocks
            .iter()
            .any(|block| block["type"] == "hashtag" && block["data"]["tag"] == "nostr");
        assert!(has_hashtag);

        // Check for link block
        let has_link = blocks
            .iter()
            .any(|block| block["type"] == "link" && block["text"] == "https://example.com");
        assert!(has_link);
