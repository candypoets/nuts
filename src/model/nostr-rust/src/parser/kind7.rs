...
impl Parser {
    pub fn parse_kind_7(&self, event: &Event) -> Result<(Kind7Parsed, Option<Vec<Request>>)> {
        ...
        // Find kind tag
        let event_kind = find_last_tag(&event.tags, "k")
            .and_then(|tag| {
                let tag_vec = tag.as_vec();
                if tag_vec.len() >= 2 {
        ...
        let target_coordinates = find_last_tag(&event.tags, "a")
            .and_then(|tag| {
                let tag_vec = tag.as_vec();
                if tag_vec.len() >= 2 {
                    Some(tag_vec[1].clone())
...
mod tests {
    ...
    fn test_parse_kind_7_emoji() {
        ...
        let tags = vec![
            Tag::parse(vec!["e".to_string(), target_event_id.to_string()]).unwrap(),
            Tag::parse(vec!["emoji".to_string(), emoji_shortcode.to_string(), emoji_url.to_string()]).unwrap(),
        ];
    ...
    fn test_parse_kind_7_no_e_tag() {
        ...
        assert!(result.unwrap_err().to_string().contains("must have at least one e tag"));
    }
