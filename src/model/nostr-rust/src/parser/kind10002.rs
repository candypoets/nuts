...
impl Parser {
    pub fn parse_kind_10002(&self, event: &Event) -> Result<(Kind10002Parsed, Option<Vec<Request>>)> {
...
mod tests {
    ...
    fn test_parse_kind_10002_with_markers() {
        ...
        let tags = vec![
            Tag::parse(vec!["r".to_string(), read_relay.to_string(), "read".to_string()]).unwrap(),
            Tag::parse(vec!["r".to_string(), write_relay.to_string(), "write".to_string()]).unwrap(),
        ];
