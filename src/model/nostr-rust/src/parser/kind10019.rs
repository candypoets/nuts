...
impl Parser {
    pub fn parse_kind_10019(&self, event: &Event) -> Result<(Kind10019Parsed, Option<Vec<Request>>)> {
...
mod tests {
    ...
    fn test_parse_kind_10019_missing_mint() {
        ...
        assert!(result.unwrap_err().to_string().contains("missing required mint tags"));
    }
