...
impl Parser {
    pub fn parse_kind_17(&self, event: &Event) -> Result<(Kind17Parsed, Option<Vec<Request>>)> {
        ...
        let reaction_type = match content.as_str() {
            ...
            _ if content.starts_with(':') && content.ends_with(':') => crate::parser::kind7::ReactionType::Emoji,
...
mod tests {
    ...
    fn test_parse_kind_17_basic() {
        ...
        let tags = vec![
            Tag::parse(vec!["r".to_string(), website_url.to_string()]).unwrap(),
        ];
        ...
        assert!(matches!(parsed.reaction_type, crate::parser::kind7::ReactionType::Like));
    ...
    fn test_parse_kind_17_no_r_tag() {
        ...
        assert!(result.unwrap_err().to_string().contains("must have an r tag"));
    }
